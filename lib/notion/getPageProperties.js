...
export default async function getPageProperties(id, block, schema, authToken, tagOptions) {
   ...
   //倒数
const time = formatDate(new Date(value?.last_edited_time).toString(), BLOG.LANG)
const editedDate = new Date(time);
...
// console.log(abc)
   ...
   properties.lastEditedTime = daysDifference
   properties.tosortTime = formatDate(new Date(value?.last_edited_time).toString(), BLOG.LANG)
   ...
   properties.tagItems = properties?.tags?.filter((tag) => tag != "Star")
      //过滤Star的标签
      .map(tag => {
