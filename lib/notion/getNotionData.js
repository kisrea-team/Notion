...
function getLatestPosts({ allPages, from, latestPostCount }) {
  ...
  const latestPosts = Object.create(allPosts).sort((a, b) => {
    const dateA = new Date(a?.tosortTime || a?.createdTime || a?.date?.start_date)
    const dateB = new Date(b?.tosortTime || b?.createdTime || b?.date?.start_date)
    return dateB - dateA
  })
...
async function getDataBaseInfoByNotionAPI({ pageId, from }) {
  ...
  //const stars = collectionData.filter(post => (post.type === 'Star' && post.status === 'Published'))
  const stars = collectionData.filter(page => page.type === 'Post' && page.status === 'Published').filter(post => post && post?.tags && post?.tags.includes('Star'))
